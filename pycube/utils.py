...
# get icon paths by name
def get_icon(icon_dir, name):
    dir_path = path.join(icon_dir,name)
    css = path.join(dir_path, name+'.css')
    ttf = path.join(dir_path, name+'.ttf')
    return {'name':name, 'css':css, 'ttf':ttf}

def get_font(font_dir, name):
    dirname = path.join(font_dir, name)
    return path.join(dirname, name+'.ttf')

def get_centered_coordinates(component, larger):
    return [int((larger.width / 2.0) - ( component.width / 2.0)), int((larger.height / 2.0) - (component.height / 2.0))]
...
def hex_to_rgb(hex):
    ...
    if hex[0] =='#':
        hex = hex[1:]
    if len(hex) != 6:
        raise ValueError(f"hex color #{hex} is not in #RRGGBB format")
    r, g, b = hex[:2], hex[2:4], hex[4:]
    r, g, b = [int(n, 16) for n in (r, g, b)]
    return (r, g, b)

def rgb_to_hex(rgb):
    '''rgb is a turple representing the color we converting to hex'''
    return '#%02x%02x%02x' % (rgb)

def add_module(module: str =""):
    path = Path().absolute()
    print(path)
    with ZipFile(module, "r") as zip:
        zip.printdir()
