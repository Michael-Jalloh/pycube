...
class IconFont(object):
    ...
    def get_icon(self, icon, size, color='black', scale='auto'):
        org_size = size
        size = max(150, size)
    ...
    def pygame_image(self, icon, size, color='black',scale='auto'):
        # we will convert the image to a bytes so that we can be able to use
        # it with pygame

        image = self.get_icon(icon,size,color,scale) # Get the icon  as an image
        size = image.size # get the size of the image
        mode = image.mode # get the mode
        data = image.tobytes() # get the image as bytes
        with open("icon.txt", "w") as f:
            for i in self.css_icons:
                f.write(f"{i}\n")
